...
import { ArrowLeft, Save, Building, Info, User, DollarSign, Box, Layers } from 'lucide-react'
...
export default function NewUnitPage() {
  ...
  const loadFormDependencies = async () => {
    ...
    try {
      ...
      // Auto select if project parameter exists
      if (projectParam) {
        setProjectId(projectParam)
      } else if (projData.success && projData.data.length > 0) {
        setProjectId(projData.data[0].id)
      }
    } catch (e) {
      console.error(e)
      toast.error('Failed to load projects/buyers lists')
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Failed to create property unit')
      }
    } catch (error) {
      console.error('Failed to create unit:', error)
      toast.error('Server error creating unit')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (loadingData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[350px] gap-3">
        ...
        <p className="text-slate-400 font-bold text-xs">Loading form dependencies...</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Add Property Unit</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-0.5">
            Register a new property apartment, shop, or parking slot in your developments portfolio
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-none shadow-sm bg-white dark:bg-slate-900 rounded-2xl p-6 space-y-4">
            ...
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 leading-relaxed">
              Upon saving, this unit will be registered to the selected development project inventory list.
            </p>
            <div className="pt-2 flex flex-col gap-3">
              ...
              >
                ...
                {submitting ? 'Creating Unit...' : 'Save Property Unit'}
              </Button>
              <Button
                type="button"
                variant="outline"
