...
export default function StaticPageEditor() {
  ...
  const fetchPageDetails = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setFaqs(page.faqs || '[]') // Hydrate FAQ state from database column

        // Resolve Template
        const currentSlug = (page.slug || '').toLowerCase()
        let resolvedTemplate = 'default'
        if (currentSlug === 'home' || currentSlug === 'homepage') {
          resolvedTemplate = 'home'
        } else if (currentSlug === 'contact' || currentSlug === 'contact-us') {
          resolvedTemplate = 'contact'
        } else if (currentSlug === 'careers') {
          resolvedTemplate = 'careers'
        } else if (currentSlug === 'terms' || currentSlug === 'terms-and-conditions' || currentSlug === 'privacy' || currentSlug === 'privacy-policy') {
          resolvedTemplate = 'policy'
        } else if (currentSlug === 'journal' || currentSlug === 'blogs' || currentSlug === 'blog') {
          resolvedTemplate = 'journal'
        } else {
          try {
        ...
        toast.error(data.error || 'Failed to load page specifications')
        router.push('/admin/n8_nwrr2675/pages')
      }
    } catch (e) {
      console.error(e)
      toast.error('Unexpected error loading page details')
    } finally {
      setLoading(false)
    }
  }
    ...
    try {
      ...
      const response = await fetch(`/api/pages/${id}`, {
        ...
        body: JSON.stringify({
          ...
          faqs // Send faqs string payload
        })
      })

      const data = await response.json()
      if (response.ok && data.success) {
        toast.success('Static page details saved successfully!')
        router.push('/admin/n8_nwrr2675/pages')
      } else {
        toast.error(data.error || 'Failed to save page modifications')
      }
    } catch (error) {
      console.error(error)
      toast.error('Server error saving page modifications')
    } finally {
      setSaving(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-neutral-950 gap-4.5 animate-pulse">
        <div className="w-10 h-10 border-4 border-slate-900 border-t-transparent rounded-full animate-spin dark:border-white"></div>
        <p className="text-xs font-bold tracking-widest uppercase text-neutral-400">Loading Page Canvas...</p>
      </div>
    )
  }

  // Hooks and helpers moved above early return block
